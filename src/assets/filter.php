...
namespace letId\assets
{
  class filter
  {
    ...
    /**
    * NOTE: avail::filter(EMAIL)->response();
    */
    public function response($filter,$flags=FILTER_NULL_ON_FAILURE)
    {
      if (is_callable($filter)) {
        // return ($filter, $this->Id, $flags);
        // return call_user_func_array(array($filter), array($this->Id, $flags));
        return call_user_func_array($filter, array($this->Id, $flags));
      } else {
