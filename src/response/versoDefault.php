		...
		*/
		...
		private function requestInitiate($menuRoute,$menuClass,$Parent)
		{
			...
			foreach ($menuRoute as $k => $v) {
				if (isset($v[$APT]) && is_string($type=$v[$APT])) {
					...
					$link = array(
						'a'=> array(
							'text' => $this->requestLinkFilter($v[$APE]), 'attr' => $this->requestAttr($v)
						)
					);
		...
		private function requestAttr($k)
		{
			...
			if (isset($k[avail::$config['APL']])) {
				$link = $k[avail::$config['APL']];
				if (preg_match(avail::$config['ATR'], $link)) {
					$attr['href'] = avail::template(array($link=>array()));
				} else {
					$attr['href'] = avail::content($link)->get($link);
					if (avail::filter($link)->response(FILTER_VALIDATE_URL, FILTER_FLAG_HOST_REQUIRED)) $attr['target'] = '_blank';
				}
				// $attr['href'] = $k[avail::$config['APL']];
				// if (avail::filter($k[avail::$config['APL']])->response(FILTER_VALIDATE_URL, FILTER_FLAG_HOST_REQUIRED)) attr['target'] = '_blank';
			} else {
